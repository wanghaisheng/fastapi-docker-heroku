...
def index() -> None:
    ...
    inputdata = input_group("usp is  fast for most, adv sitemap is plan B,insane crawl is your last straw for those dont have /robots.txt file ", [
        ...
        radio('cloud backup',['googlesheet','no'],inline=True,value='no',name='cloudbackup')
    ...
    q1 = inputdata['q1']
    cloudbackup=inputdata['cloudbackup']
    # if not isvaliddomain(url):
    #     return {"urls": 'not a valid domain'}
    if url.startswith("http://"):
        pass
    ...
    if not validators.url(url):
        ...
        with use_scope('loading'):
            ...
            if backend=='supabase':
                from src.supabase.dbmanipulation import Query_urls_list_In_Db,Update_urls_list_In_Db
            elif backend=='sqlite':
                from src.sqlite.dbmanipulation import Query_urls_list_In_Db,Update_urls_list_In_Db
            ...
            # put_logbox('log',200)
            if len(urls) > 0:
                encoded_urls = 'label,reference,url\n'
                encoded_media = 'label,reference,url\n'
                encoded_others = 'label,reference,url\n'
                if cloudbackup=='googlesheet':
                    client = Client()
                    spread = Spread('shopify-urls')
                ...
                for idx, item in enumerate(urls):
                    if cloudbackup=='googlesheet':
                        ...
                        df['id']=str(idx)
                        df['url']=item
                        spread.df_to_sheet(df, index=False, sheet=url, start='A2', replace=True)


                    if '.jpg' in item or '.png' in item or '.mp4' in item:
                        encoded_media+=str(idx)+','+item+','+item
                        encoded_media = encoded_media+'\n'                        
                    elif 'product' in item or 'blog' in 'item' or 'collection' in 'item' or 'page' in 'item':
                    # encoded = encoded+','.join(list(i))

                        encoded_urls+=str(idx)+','+item+','+item
                        encoded_urls = encoded_urls+'\n'
                    else:
                        
                        encoded_others+=str(idx)+','+item+','+item
                        encoded_others = encoded_others+'\n'

                array_media=encoded_media.encode('utf-8')
                array_urls=encoded_urls.encode('utf-8')
                array_others=encoded_others.encode('utf-8')
                ...
                put_row([
                    ...
                    put_file(urlparse(url).netloc+'.txt',
                            array_urls, 'download collection/page/product/blog urls'),
                    put_file(urlparse(url).netloc+'.txt',
                            array_media, 'download all image/videos'),
                    put_file(urlparse(url).netloc+'.txt',
                            array_others, 'download other')                                  
                ])
...
# start_server([index,feedback,privacy,terms],port =5001,static_dir='assets',remote_access =True, auto_open_webbrowser=True)
